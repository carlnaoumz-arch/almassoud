...
export function MenuBrowser() {
 ...
 // Preserve text entered into the server-rendered field before hydration finishes.
 useEffect(()=>{setQuery(searchInput.current?.value ?? '');},[]);
 const clearSearch=()=>{if(searchInput.current)searchInput.current.value='';setQuery('');};
 ...
 return <>
  <div className="menu-tools"><label className="dish-search"><Search size={18}/><input ref={searchInput} aria-label="Search dishes or ingredients" type="search" defaultValue="" onChange={e=>setQuery(e.target.value)} placeholder="Find your favourite dish…"/>{query&&<button aria-label="Clear search" onClick={clearSearch}><X size={18}/></button>}</label><span className="menu-result" role="status">{query?`${found} ${found===1?'match':'matches'}`:`${itemCount} dishes & drinks`}</span></div>
  <div className="menu-workspace">
   <aside className="menu-index"><p>ON THE MENU</p><nav aria-label="Menu categories"><a href="#menu-top" onClick={()=>flushSync(clearSearch)}>All categories <span>{itemCount}</span></a>{menuSections.map(section=><a key={section.id} href={'#'+section.id} onClick={()=>flushSync(clearSearch)}>{section.title}<span>{section.items.length}</span></a>)}</nav><a className="original-menu" href="/menus/diner.pdf" download>Original menu <ArrowUpRight size={15}/></a></aside>
   ...
   </section>)}{!visible.length&&<div className="menu-empty"><h2>No dishes found</h2><p>Try another dish or ingredient.</p><button onClick={clearSearch}>Show the full menu</button></div>}</div>
  </div>
 </>;
}
